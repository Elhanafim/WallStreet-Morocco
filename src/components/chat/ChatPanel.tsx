...
function renderMarkdown(text: string): React.ReactNode {
  ...
  lines.forEach((line, i) => {
    ...
    if (/^#{2,3}\s/.test(line)) {
      ...
      nodes.push(
        <p key={key} className="font-bold text-sm mt-2 mb-0.5" style={{ color: "#1d4ed8" }}>
    ...
    if (/^[-*•]\s/.test(line)) {
      ...
      nodes.push(
        <div key={key} className="flex gap-2 items-start leading-snug my-0.5">
          <span className="mt-0.5 shrink-0 w-1.5 h-1.5 rounded-full bg-[#1d4ed8] mt-1.5" />
    ...
    if (numMatch) {
      nodes.push(
        <div key={key} className="flex gap-2 items-start leading-snug my-0.5">
          <span className="shrink-0 font-bold text-[#1d4ed8] text-xs w-4">{numMatch[1]}.</span>
...
function inlineMarkdown(text: string): React.ReactNode {
  ...
  return parts.map((part, j) => {
    ...
    if (linkMatch) {
      return (
        <a key={j} href={linkMatch[2]} className="text-[#1d4ed8] underline" target="_blank" rel="noopener noreferrer">
          {linkMatch[1]}
        </a>
      );
    }
  ...
  return (
    <div className="flex items-center gap-1 px-1 py-1">
      {[0, 1, 2].map((i) => (
        <span
          key={i}
          className="w-2 h-2 rounded-full bg-[#1d4ed8] animate-bounce opacity-70"
...
export default function ChatPanel({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label="Assistant IA WallStreet Morocco"
    >
      {/* ── Header ── */}
      <div
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="font-bold text-sm text-white leading-tight">Assistant IA</p>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2.5 text-sm">
        ...
        {messages.length === 0 && (
          <div className="flex gap-2 items-start">
            ...
            <div>
              <p className="text-[10px] font-semibold mb-1" style={{ color: "#1d4ed8" }}>
                Assistant IA
              </p>
            ...
            <div className={msg.role === "assistant" ? "" : ""}>
              ...
              <div
                className={`rounded-2xl px-3 py-2 max-w-[85%] leading-relaxed ${
                  msg.role === "user"
                    ? "bg-[#1d4ed8] text-white rounded-tr-sm ml-auto"
              ...
              <div className="bg-gray-50 border border-gray-100 rounded-2xl rounded-tl-sm px-3 py-2 max-w-[85%] text-gray-800 leading-relaxed">
                {streamingContent ? (
                  <>
                    ...
                    <span className="inline-block w-0.5 h-3 bg-[#1d4ed8] animate-pulse ml-0.5 align-middle" />
                  </>
                ) : (
                  <TypingIndicator />
                )}
      ...
      {showDonateNudge && !isStreaming && context.currentPage !== "/donate" && (
        <div className="mx-3 mb-2 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-xs">
          ...
          <a
            href="/donate"
            onClick={onHideDonateNudge}
            className="mt-1.5 inline-block font-semibold text-[#1d4ed8] hover:underline"
          >
            Faire un don →
          </a>
        </div>
      ...
      <div className="flex items-center justify-center gap-1 py-1.5 border-t border-gray-100">
        <span className="text-[10px] text-gray-400">◈ WallStreet Morocco · Propulsé par</span>
