...
function CopyButton({ value, label }: { value: string; label: string }) {
  ...
  return (
    ...
    >
      {toast === 'copied' ? (
        <span className="text-xs font-medium text-emerald-600 whitespace-nowrap">{t('copied')}</span>
      ) : toast === 'error' ? (
        <span className="text-xs font-medium text-red-500 whitespace-nowrap">{t('copyError')}</span>
      ) : (
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-gray-100 last:border-0">
      ...
      <span className="text-sm font-medium text-gray-900 flex-1 text-right break-all">{value}</span>
  ...
  return (
    <div className="pt-16 min-h-screen bg-gray-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
              <div className="p-6 border-b border-gray-100">
                ...
                <h2 className="text-xl font-medium text-gray-900 mt-2">Debit / Credit Card · Apple Pay</h2>
              ...
              <div className="p-6 flex flex-col items-center border-b border-gray-100">
                ...
                />
                <p className="mt-3 font-medium text-gray-900 text-sm">{REVOLUT.username}</p>
                <a
                  href={REVOLUT.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-3 inline-flex items-center gap-2 bg-gray-900 text-white text-sm font-medium px-5 py-2.5 rounded-xl hover:bg-gray-700 transition-colors"
                >
                  Support here →
                </a>
              </div>
              ...
              <div className="p-4">
                <button
                  ...
                  className="w-full text-left text-xs font-medium text-gray-600 hover:text-gray-900 transition-colors flex items-center gap-1"
                >
                  {revolutOpen
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
              <div className="p-6 border-b border-gray-100">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-medium px-2.5 py-1 rounded-full text-white" style={{ backgroundColor: '#c1272d' }}>
                    🇲🇦 {t('morocco')}
                  </span>
                </div>
                <h2 className="text-xl font-medium text-gray-900 mt-2">Attijariwafa Bank</h2>
                <p className="text-xs text-gray-500 mt-1">
                  {displayAmount
              ...
              <div className="p-6 flex flex-col items-center border-b border-gray-100">
                <img
                  ...
                  className="w-48 h-48 rounded-xl object-cover"
                />
                <p className="mt-3 font-medium text-gray-900 text-sm">{ATTIJARI.beneficiary}</p>
      ...
      <section className="pb-14 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            <h3 className="text-base font-medium text-gray-900 mb-4">{t('suggestedAmount')}</h3>
      ...
      <section className="py-14 px-4 bg-white border-t border-gray-100">
        <div className="max-w-4xl mx-auto">
          <h3 className="text-xl font-medium text-gray-900 text-center mb-10">{t('whyTitle')}</h3>
      ...
      <section className="py-16 px-4 bg-gray-50 border-t border-gray-100">
        <div className="max-w-2xl mx-auto text-center">
          <p className="text-2xl font-medium text-gray-900 mb-5">{t('thankYou')}</p>
