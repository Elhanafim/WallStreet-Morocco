...
function CreatePortfolioModal({
  ...
  return (
    ...
    >
      <div className="bg-[var(--bg-surface)] rounded-lg shadow-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-medium text-primary">Nouveau portefeuille</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              ...
              className="flex-1 px-4 py-3 rounded-xl bg-secondary text-white font-medium text-sm hover:bg-secondary-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
            >
              {loading ? (
...
function DeleteModal({
  ...
  return (
    <div
      ...
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div className="bg-[var(--bg-surface)] rounded-lg shadow-xl w-full max-w-sm p-6 text-center">
        <div className="w-14 h-14 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
          <Trash2 className="w-6 h-6 text-red-500" />
        </div>
        <h2 className="text-lg font-medium text-primary mb-2">Supprimer ce portefeuille ?</h2>
...
function PortfolioCard({
  ...
  return (
    <div className="bg-[var(--bg-surface)] rounded-lg border border-[var(--border)] hover:scale-[1.01] hover:shadow-lg transition-all duration-200 p-6">
      {/* Header */}
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-medium text-primary text-base leading-tight">{portfolio.name}</h3>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-xs font-medium text-secondary/70 bg-secondary/10 px-2 py-0.5 rounded-full">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-surface-50 rounded-xl p-3">
          ...
          <p className="font-medium text-primary text-base">{fmtMAD(invested)}</p>
        </div>
        <div className="bg-surface-50 rounded-xl p-3">
          <p className="text-xs text-primary/40 mb-1 flex items-center gap-1">
            <BarChart2 className="w-3 h-3" /> Positions
          </p>
          <p className="font-medium text-primary text-base">{portfolio.holdings.length}</p>
        </div>
      </div>
      ...
      <Link
        ...
        className="w-full flex items-center justify-center gap-2 bg-primary text-white font-medium py-3 rounded-xl hover:bg-secondary transition-colors text-sm"
      >
...
function EmptyState({ onCreate }: { onCreate: () => void }) {
  return (
    <div className="text-center py-20">
      ...
      <h3 className="text-xl font-medium text-primary mb-2">Aucun portefeuille</h3>
      ...
      <button
        onClick={onCreate}
        className="inline-flex items-center gap-2 bg-secondary text-white font-medium px-6 py-3.5 rounded-xl hover:bg-secondary-600 transition-colors shadow-md"
      >
...
function SkeletonCard() {
  return (
    <div className="bg-[var(--bg-surface)] rounded-lg border border-[var(--border)] p-6 space-y-4">
      <div className="flex items-center gap-3">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <button
              onClick={() => setShowCreate(true)}
              className="bg-[var(--bg-surface)] rounded-lg border-2 border-dashed border-[var(--border)] hover:border-[var(--gold)] hover:bg-[var(--bg-elevated)] transition-all duration-200 p-6 flex flex-col items-center justify-center gap-3 text-primary/40 hover:text-secondary min-h-[200px]"
            >
              <Plus className="w-8 h-8" />
              <span className="font-medium text-sm">Nouveau portefeuille</span>
            </button>
          </div>
        )}
      </div>
