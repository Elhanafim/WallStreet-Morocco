    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-[#0A2540] flex items-center justify-center text-white text-2xl font-black flex-shrink-0">
            {user?.name?.[0]?.toUpperCase() ?? 'U'}
          </div>
          <div className="flex-1 min-w-0">
            <h2 className="text-xl font-black text-[#0A2540] truncate">{user?.name ?? 'Utilisateur'}</h2>
        ...
        {[
          ...
          <div key={label} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm font-semibold text-[#0A2540] truncate">{value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Edit Name */}
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <h3 className="text-base font-black text-[#0A2540] mb-4">Modifier le nom</h3>
        <form onSubmit={handleNameSave} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-[#0A2540] mb-1.5">Nom complet</label>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-5 py-2.5 bg-[#0A2540] text-white text-sm font-bold rounded-xl hover:bg-[#3A86FF] disabled:opacity-50 transition-colors flex items-center gap-2"
            >
              {nameForm.saving ? (
      ...
      {/* Change Password */}
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <h3 className="text-base font-black text-[#0A2540] mb-4">Changer le mot de passe</h3>
      ...
      {/* Subscription */}
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <h3 className="text-base font-black text-[#0A2540] mb-4">Abonnement</h3>
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Shield className="w-4 h-4 text-[#3A86FF]" />
              <span className={`text-sm font-bold px-2.5 py-0.5 rounded-full ${badge.className}`}>{badge.label}</span>
            </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-red-100">
        <h3 className="text-base font-black text-red-600 mb-1">Zone de danger</h3>
        <p className="text-sm text-gray-500 mb-4">Ces actions sont irréversibles. Agissez avec précaution.</p>
        <button
          onClick={() => setShowDeleteConfirm(true)}
          className="flex items-center gap-2 px-4 py-2.5 border border-red-200 text-red-600 text-sm font-semibold rounded-xl hover:bg-red-50 transition-colors"
        >
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
            ...
            <h3 className="text-lg font-black text-[#0A2540] text-center mb-2">Supprimer le compte ?</h3>
